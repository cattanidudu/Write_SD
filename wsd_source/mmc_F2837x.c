...
// new function made by dudu
static
void xmit_spi_float (BYTE dat)    //EDITED
{
    volatile DWORD rcvdat;                      //included file like integer.h for DWORD definition
    ...
    SpiaRegs.SPITXBUF = ((DWORD)dat & 0xFF00);        //Write data
...
// new function made by dudu_
DRESULT disk_write_float (
    BYTE drv,            /* Physical drive nmuber (0) */
    const BYTE *buff,    /* Pointer to the data to be written */
    DWORD sector,        /* Start sector number (LBA) */
    BYTE count            /* Sector count (1..255) */
)
{
    ...
    {
        if ((send_cmd(CMD24, sector) == 0)    /* WRITE_BLOCK */
                && xmit_datablock_float(buff, 0xFE))
            count = 0;
    }
    else                  /* Multiple block write */
    {
